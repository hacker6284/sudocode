...
impl Emitter<'_> {
    ...
    fn py_ty(&self, ty: &Ty) -> String {
        match ty {
            ...
            Ty::Record(n) | Ty::Enum(n) => self.type_qual(n),
            _ => "object".into(),
        }
    }
    ...
    fn run(mut self, with_tests: bool) -> String {
        ...
        for r in &self.m.records {
            ...
            for f in &r.fields {
                let fname = &f.name;
                let fty = &f.ty;
                self.line(1, &format!("{fname}: {}", self.py_ty(fty)));
            }
            self.blank();
        }
        for e in &self.m.enums {
            for v in &e.variants {
                ...
                self.line(
                    ...
                    &format!(
                        "class {}:",
                        sudoc_ir::mangle::variant_class(&e.name, &v.name)
                    ),
                );
                self.line(
                    1,
                    &format!("_sudo_kind = (\"e\", \"{}.{}\")", e.name, v.name),
                );
                for f in &v.fields {
                    let fname = &f.name;
                    let fty = &f.ty;
            ...
            let union: Vec<String> = e
                .variants
                .iter()
                .map(|v| sudoc_ir::mangle::variant_class(&e.name, &v.name))
                .collect();
        ...
        match place {
            ...
            Place::Index {
                ...
                match strip(base_ty) {
                    ...
                    Ty::Map(..) => self.line(depth, &format!("_rt.map_put({b}, {i}, {value})")),
    ...
    fn variant_field_ty(&self, enum_name: &str, variant: &str, i: usize) -> Option<Ty> {
        ...
        let e = sudoc_ir::find_enum(self.all, enum_name)
            .unwrap_or_else(|| panic!("internal error: enum '{enum_name}' not found in program"));
        let v = e
            .variants
            .iter()
        ...
        match p {
            ...
            IrPattern::Variant {
                ...
                let cls = match (enum_name.as_str(), variant.as_str()) {
                    ...
                    _ => self.variant_qual(enum_name, variant),
                };
...
fn py_str(scalars: &[i64]) -> String {
    ...
    for &c in scalars {
        match char::from_u32(c as u32) {
            ...
            Some(ch) if (ch as u32) <= 0xFFFF => s.push_str(&format!("\\u{:04x}", ch as u32)),
...
pub fn emit_api(m: &IrModule, all: &[IrModule]) -> Option<String> {
    ...
    push(
        w,
        &format!(
            "\"\"\"Host-facing API for {}.sudo — generated by sudoc. Traps raise",
            m.name
        ),
    );
    push(
            ...
            match &p.boundary {
                ...
                Te::Named(_) => {
                    ...
                    if let Ty::Record(rname) = &p.ty {
                        let r = sudoc_ir::find_record(all, rname).unwrap_or_else(|| {
                            panic!("internal error: record '{rname}' not found in program")
                        });
                        for f in &r.fields {
                            let fname = &f.name;
...
fn conv_in(te: &sudoc_ir::BoundaryTy, var: &str) -> Option<String> {
    ...
    match te {
        ...
        Te::Map(k, v) => {
            ...
            Some(format!(
                "_rt.host_map({var}, lambda _k: {kc}, lambda _v: {vc})"
            ))
        }
        Te::Set(t) => {
        ...
        Te::Tuple(ts) => {
            ...
            let parts: Vec<String> = ts
                .iter()
                .enumerate()
                .map(|(i, t)| conv_in(t, &format!("_t[{i}]")).unwrap_or_else(|| format!("_t[{i}]")))
                .collect();
            Some(format!(
...
fn conv_out(te: &sudoc_ir::BoundaryTy, var: &str) -> Option<String> {
    ...
    match te {
        ...
        Te::Map(k, v) => {
            ...
            let vc = conv_out(v, "_v").unwrap_or_else(|| "_v".into());
            Some(format!(
                "_rt.out_map({var}, lambda _k: {kc}, lambda _v: {vc})"
            ))
        }
        Te::Set(t) => {
            let c = conv_out(t, "_v").unwrap_or_else(|| "_v".into());
