//! Execution tests: generated Swift actually compiles and its sudo tests pass
//! under swiftc. Mirrors backend_js/tests/execute.rs.

use std::path::Path;
use std::process::Command;

fn run_generated(name: &str, src: &str) -> std::process::Output {
    let dir = std::env::temp_dir().join(format!("sudoc-swifttest-{name}-{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
    let ir = sudoc_types::check_source(src, name).expect("checks");
    let code = sudoc_backend_swift::emit(&ir, true);
    let entry = format!("{name}.swift");
    std::fs::write(dir.join(&entry), &code).unwrap();
    std::fs::write(dir.join(sudoc_backend_swift::RUNTIME_FILE), sudoc_backend_swift::RUNTIME)
        .unwrap();
    let build = Command::new("swiftc")
        ...
        .args([
            ...
            "sudo_tests",
            &entry,
            sudoc_backend_swift::RUNTIME_FILE,
        ])
        .output()
        .expect("swiftc runs");
    ...
    let out = Command::new("./sudo_tests")
        .current_dir(&dir)
        .output()
        .expect("sudo_tests runs");
...
#[test]
fn all_examples_pass_their_tests_in_swift() {
    let examples = Path::new(env!("CARGO_MANIFEST_DIR")).join("../../../examples");
    let mut checked = 0;
    for path in walk(&examples) {
        let name = path.file_stem().unwrap().to_str().unwrap().to_string();
        // The deliberately order-dependent pitfall may pass or trap under Swift —
