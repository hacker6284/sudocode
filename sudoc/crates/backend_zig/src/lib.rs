...
use sudoc_ir::never_written::{compute_address_taken, expr_root_var, func_is_address_taken};
use sudoc_ir::{
...
struct Emitter<'a> {
    ...
    /// Extra types to declare in this file (composites owned by `sudo_types`).
    extra: Option<&'a TypeSet>,
...
impl<'a> Emitter<'a> {
    ...
    fn run(mut self, with_tests: bool) -> String {
        ...
        // needs `st` (`sudo_types.zig`, only when the IR has that module).
        let mut types = collect_types(self.m);
        if let Some(ex) = self.extra {
            types.types.extend(ex.types.clone());
        }
        ...
        // Monomorphized types + copy/eq/canon helpers.
    ...
    fn emit_type_section(&mut self, types: &TypeSet) {
        ...
        let order = topo_order(&local, self.all);
        for name in &order {
            ...
            match ty {
                ...
                Ty::Tuple(ts) => {
                    let n = ts.len();
                    if !(2..=8).contains(&n) {
                        panic!("internal error: Zig runtime has SudoTuple2..=8, got arity {n}");
                    }
                    let args: Vec<String> = ts.iter().map(zig_ty).collect();
                    self.line(&format!(
                        "pub const {name} = rt.SudoTuple{n}({});",
                        args.join(", ")
                    ));
                }
                Ty::Option_(inner) => {
                ...
                Ty::Result_(ok, err) => {
                    ...
                    self.line(&format!(
                        "pub const {name} = rt.SudoResult({ok_z}, {err_z});"
                    ));
                }
                Ty::Map(k, v) => {
                    self.line(&format!(
        ...
        // Shared nominals (and composites that mention them) live in
        // `sudo_types.zig`; alias so bare mangled names resolve there.
        ...
        // keyapp/copy/eq for a hoisted key live in `st`; alias them so a
        // local Map/Set can name that key.
        let key_types = collect_key_types(types, self.all);
    ...
    fn copy_uses_alloc(&self, ty: &Ty) -> bool {
        match ty {
            ...
            Ty::Record(rname) => self.record(rname).fields.iter().any(|f| needs_dup(&f.ty)),
            Ty::Enum(ename) => self.enum_(ename).variants.iter().any(|v| {
                v.fields
                    .iter()
    ...
    fn emit_type_ops(&mut self, name: &str, ty: &Ty) {
        ...
        match ty {
            Ty::List(elem) => {
                ...
                self.line(&format!(
                    "out.append({elem_copy}) catch @panic(\"sudo: arena OOM\");"
                ));
                self.indent -= 1;
                self.line("}");
                    ...
                    if v.fields.is_empty() {
                        ...
                        for f in &v.fields {
                            ...
                            let field = if boxed_in_payload(fty) {
                                ...
                                format!(".{fname} = rt.box(alloc, {}, {inner}),", zig_ty(fty))
                            } else if needs_dup(fty) {
    ...
    fn emit_keyapp_value(&mut self, ty: &Ty, expr: &str) {
        match ty {
            ...
            _ => self.line(&format!("{}({expr});", keyapp_fn(ty))),
        }
    }
                ...
                } = target
                {
                    match base_ty {
                        Ty::List(_) => {
                            ...
                            let val = self.store_to(value, &format!("{base_lv}.alloc"), esc);
                        ...
                        Ty::Map(..) => {
                            ...
                            let key = self.store_to(index, &format!("{base_lv}.alloc"), esc);
                            let val = self.store_to(value, &format!("{base_lv}.alloc"), esc);
                ...
                let (lhs, val) = match target {
                    ...
                    Place::Field { base, .. } => {
                        ...
                        let val = if is_container_ty(&value.ty) {
                            ...
                            let is_inout =
                                place_root_local(base).is_some_and(|n| self.inouts.contains(n));
                            if is_inout {
                                self.store_to(value, "_sudo_ret_alloc", true)
                            } else {
                                let dest = self.scratch_alloc();
    ...
    fn emit_expect_trap(&mut self, kind: &str, body: &[IrStmt], line: u32) {
        ...
        self.indent -= 1;
        self.line(&format!(
            "}} else return rt.expectTrapNone({line}, \"{kind}\");"
        ));
    }
    ...
    fn emit_match(&mut self, scrutinee: &IrExpr, arms: &[IrMatchArm]) {
        ...
        match &scrutinee.ty {
            Ty::Int | Ty::Bool => {
                ...
                let bool_exhaustive = matches!(scrutinee.ty, Ty::Bool) && saw_true && saw_false;
                            ...
                            if vdef.fields.is_empty() {
                                ...
                                for (i, f) in vdef.fields.iter().enumerate() {
                                    ...
                                    if boxed_in_payload(fty) {
                                        ...
                                        self.line(&format!("const {b} = {variant}_p.{fname};"));
                                    }
                                    if !binder_used(&arm.body, b) {
                                        self.line(&format!("_ = {b};"));
                                    }
    ...
    fn emit_assert(&mut self, cond: &IrExpr, line: u32) {
        ...
        } = &cond.kind
        {
            ...
            if !is_scalar(&lhs.ty) && !matches!(lhs.ty, Ty::Func { .. } | Ty::Map(..) | Ty::Set(_))
            {
                let l = self.expr(lhs);
                let r = self.expr(rhs);
    ...
    fn expr(&mut self, e: &IrExpr) -> String {
        match &e.kind {
            ...
            IrExprKind::Text(chars) => {
                ...
                for c in chars {
                    self.line(&format!(
                        "{t}.append({c}) catch @panic(\"sudo: arena OOM\");"
                    ));
                }
                t
            }
            ...
            IrExprKind::List(xs) => {
                ...
                for x in xs {
                    let v = self.store(x);
                    self.line(&format!(
                        "{t}.append({v}) catch @panic(\"sudo: arena OOM\");"
                    ));
                }
                t
            }
    ...
    fn emit_list_concat(&mut self, lhs: &IrExpr, rhs: &IrExpr, ty: &Ty) -> String {
        ...
        for src in [&lt, &rt] {
            ...
            self.indent += 1;
            self.line(&format!(
                "{out}.append({copied}) catch @panic(\"sudo: arena OOM\");"
            ));
            self.indent -= 1;
            self.line("}");
        }
    ...
    fn emit_builtin(&mut self, b: Builtin, args: &[IrExpr], result_ty: &Ty) -> String {
        match b {
            ...
            Builtin::Filled => {
                ...
                self.indent += 1;
                self.line(&format!(
                    "{t}.append({v_t}) catch @panic(\"sudo: arena OOM\");"
                ));
                self.indent -= 1;
                self.line("}");
                t
            ...
            Builtin::ResIsOk => {
                ...
                let t = self.tmp("isok");
                self.line(&format!(
                    "const {t}: bool = switch ({a}) {{ .Ok => true, .Err => false }};"
                ));
                t
            }
            Builtin::ResIsErr => {
                ...
                self.line(&format!(
                    "const {t}: bool = switch ({a}) {{ .Ok => false, .Err => true }};"
                ));
                t
            }
            Builtin::ResUnwrap => {
            ...
            Builtin::MapDelete | Builtin::SetAdd | Builtin::SetRemove => unreachable_shape(
                &format!("builtin {b:?} as non-mutating (expected MutBuiltin)"),
            ),
        }
    }
    ...
    fn emit_mut_builtin(
        ...
        match b {
            ...
            Builtin::ListPop => self.tryx(&format!("{recv_lv}.pop()")),
            Builtin::ListInsert => {
...
fn mentions_name(ty: &Ty, names: &BTreeSet<String>) -> bool {
    match ty {
        Ty::Record(n) | Ty::Enum(n) => names.contains(n),
        Ty::List(e) | Ty::Set(e) | Ty::Option_(e) => mentions_name(e, names),
        Ty::Map(k, v) | Ty::Result_(k, v) => mentions_name(k, names) || mentions_name(v, names),
        Ty::Tuple(ts) => ts.iter().any(|t| mentions_name(t, names)),
        Ty::Func { params, ret } => {
            params.iter().any(|t| mentions_name(t, names))
                || ret.as_ref().is_some_and(|r| mentions_name(r, names))
        }
        _ => false,
    }
}
...
/// Portable composites compose `rt.key_*` so two modules share one function
/// value (and therefore one Map/Set type). Nominals keep a per-decl `keyapp_*`.
fn keyapp_fn(ty: &Ty) -> String {
    match ty {
        Ty::Int => "rt.key_i64".into(),
        Ty::Bool => "rt.key_bool".into(),
        Ty::Float => unreachable_shape("float map/set key"),
        Ty::List(e) => format!("rt.key_list({}, {})", zig_ty(e), keyapp_fn(e)),
        Ty::Tuple(ts) => {
            let n = ts.len();
            if !(2..=8).contains(&n) {
                panic!("internal error: Zig runtime has key_tuple2..=8, got arity {n}");
            }
            let targs: Vec<String> = ts.iter().map(zig_ty).collect();
            let kargs: Vec<String> = ts.iter().map(keyapp_fn).collect();
            format!(
                "rt.key_tuple{n}({}, {})",
                targs.join(", "),
                kargs.join(", ")
        ...
        Ty::Option_(inner) => {
            if boxed_in_payload(inner) {
                format!("rt.key_option_box({}, {})", zig_ty(inner), keyapp_fn(inner))
            } else {
                format!("rt.key_option({}, {})", zig_ty(inner), keyapp_fn(inner))
            }
        }
        Ty::Result_(ok, err) => {
            let ok_stored = if boxed_in_payload(ok) {
                format!("*const {}", zig_ty(ok))
            } else {
                zig_ty(ok)
            };
            let err_stored = if boxed_in_payload(err) {
                format!("*const {}", zig_ty(err))
            } else {
                zig_ty(err)
            };
            let ko = if boxed_in_payload(ok) {
                format!("rt.key_box({}, {})", zig_ty(ok), keyapp_fn(ok))
            } else {
                keyapp_fn(ok)
            };
            let ke = if boxed_in_payload(err) {
                format!("rt.key_box({}, {})", zig_ty(err), keyapp_fn(err))
            } else {
                keyapp_fn(err)
            };
            format!("rt.key_result({ok_stored}, {err_stored}, {ko}, {ke})")
        }
        Ty::Record(_) | Ty::Enum(_) => format!("keyapp_{}", mangle(ty)),
        _ => unreachable_shape("non-hashable map/set key"),
    }
}
...
fn collect_key_types(types: &TypeSet, all: &[IrModule]) -> Vec<String> {
    fn add_composite(ty: &Ty, all: &[IrModule], out: &mut BTreeSet<String>) {
        match ty {
            Ty::Int | Ty::Bool | Ty::Float => {}
            Ty::List(e) | Ty::Option_(e) => add_composite(e, all, out),
            Ty::Tuple(ts) => {
                for t in ts {
                    add_composite(t, all, out);
                }
            }
            Ty::Result_(a, b) => {
                add_composite(a, all, out);
                add_composite(b, all, out);
            }
            Ty::Record(n) if out.insert(mangle(ty)) => {
                let r = sudoc_ir::find_record(all, n)
                    .unwrap_or_else(|| panic!("internal error: record '{n}' not found in program"));
                for f in &r.fields {
                    add_composite(&f.ty, all, out);
                }
            }
            Ty::Enum(n) if out.insert(mangle(ty)) => {
                let e = sudoc_ir::find_enum(all, n)
                    .unwrap_or_else(|| panic!("internal error: enum '{n}' not found in program"));
                for v in &e.variants {
                    for f in &v.fields {
                        add_composite(&f.ty, all, out);
                    }
    ...
    for ty in types.types.values() {
        match ty {
            Ty::Map(k, _) => add_composite(k, all, &mut out),
            Ty::Set(e) => add_composite(e, all, &mut out),
            _ => {}
        }
    }
...
fn binder_used(stmts: &[IrStmt], name: &str) -> bool {
    ...
    stmts.iter().any(|s| match s {
        ...
        IrStmt::If { arms, else_block } => {
            arms.iter()
                .any(|(c, b)| expr_uses(c, name) || binder_used(b, name))
...
fn stmt_allocates(s: &IrStmt) -> bool {
    match s {
        ...
        IrStmt::TupleAssign { value, .. } => needs_dup(&value.ty) || expr_allocates(value),
...
fn topo_order(set: &TypeSet, all: &[IrModule]) -> Vec<String> {
    fn complete_deps(ty: &Ty, all: &[IrModule], out: &mut Vec<Ty>) {
        match ty {
            Ty::List(e) => {
                if !is_scalar(e) {
                    out.push((**e).clone());
            ...
            Ty::Record(name) => {
                let r = sudoc_ir::find_record(all, name).unwrap_or_else(|| {
                    panic!("internal error: record '{name}' not found in program")
                });
                out.extend(r.fields.iter().map(|f| f.ty.clone()));
            }
            Ty::Enum(name) => {
                let e = sudoc_ir::find_enum(all, name).unwrap_or_else(|| {
                    panic!("internal error: enum '{name}' not found in program")
                });
                for v in &e.variants {
                    for f in &v.fields {
                        out.push(f.ty.clone());
    ...
    fn visit(
        ...
        complete_deps(ty, all, &mut deps);
        for d in deps {
            let dn = mangle(&d);
            if set.types.contains_key(&dn) {
                visit(&dn, set, all, done, order);
            }
        }
        order.push(name.to_string());
    }
    for name in set.types.keys() {
        visit(name, set, all, &mut done, &mut order);
    }
    order
}
...
impl Backend for ZigBackend {
    ...
    fn emit_program(
        ...
        let shared_nominals: BTreeSet<String> = modules
            .iter()
            .find(|m| m.name == sudoc_ir::mangle::SHARED_TYPES_MODULE)
            .map(|st| {
                st.records
                    .iter()
                    .map(|r| r.name.clone())
                    .chain(st.enums.iter().map(|e| e.name.clone()))
                    .collect()
            })
            .unwrap_or_default();

        let mut hoisted: BTreeSet<String> = shared_nominals.clone();
        let extra_shared = if shared_nominals.is_empty() {
            TypeSet::default()
        } else {
            let all_types = collect_types_all(&modules);
            for (k, ty) in &all_types.types {
                if mentions_name(ty, &shared_nominals) {
                    hoisted.insert(k.clone());
                }
            }
            TypeSet {
                types: all_types
                    ...
                    .iter()
                    .filter(|(k, ty)| {
                        hoisted.contains(*k) && !matches!(ty, Ty::Record(_) | Ty::Enum(_))
                    })
                    .map(|(k, v)| (k.clone(), v.clone()))
                    .collect(),
            }
        ...
        let mut out = Vec::new();
        for m in deps {
            let is_shared = m.name == sudoc_ir::mangle::SHARED_TYPES_MODULE;
            let (h, extra) = if is_shared {
                (&empty_hoist, Some(&extra_shared))
            } else {
                (&hoisted, None)
            };
            out.push(GeneratedFile {
                path: format!("{}.zig", m.name),
                contents: emit_with(m, &modules, false, false, h, extra),
            });
        }
        out.push(GeneratedFile {
            path: format!("{}.zig", entry.name),
            contents: emit_with(entry, &modules, with_tests, true, &hoisted, None),
        });
        Ok(out)
    }
