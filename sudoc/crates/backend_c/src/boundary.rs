...
fn adaptable(f: &IrFunc) -> bool {
    f.params.iter().all(|p| {
        shape(&p.boundary)
            .is_some_and(|s| !p.inout || matches!(s, Shape::ListInt | Shape::ListFloat))
    }) && f
        .ret_boundary
        .as_ref()
        .map(|r| shape(r).is_some())
        .unwrap_or(true)
}
...
pub(crate) fn emit_wrappers(m: &IrModule, out: &mut String) {
    ...
    for f in exports {
        ...
        let _ = writeln!(
            out,
            "    if (setjmp(sudo_trap_jmp) != 0) return sudo_trap_status;"
        );
        ...
        for p in &f.params {
            match shape(&p.boundary).expect("adaptable") {
                ...
                Shape::Text => {
                    ...
                    let _ = writeln!(out, "    {lt} _{0} = {lt}_from(_{0}_buf, _{0}_n);", p.name);
        ...
        if f.ret.is_some() {
            ...
            match shape(rb).expect("adaptable") {
                ...
                s @ (Shape::ListInt | Shape::ListFloat) => {
                    ...
                    let _ = writeln!(
                        out,
                        "    memcpy(*out, _r.data, (size_t)_r.len * sizeof({et}));"
                    );
...
pub fn emit_header(m: &IrModule) -> Option<String> {
    ...
    let _ = writeln!(
        out,
        " * Every call returns SUDO_OK or the trap kind that aborted it."
    );
    let _ = writeln!(
        out,
        " * Out-buffers (char*, arrays) are malloc'd; free() them when done."
    );
    let _ = writeln!(
        out,
        " * Not thread-safe: calls share one trap jump buffer. */"
    );
    ...
    if !skipped.is_empty() {
        ...
        let _ = writeln!(
            out,
            "/* Exports without a portable C mapping yet (use the internal"
        );
