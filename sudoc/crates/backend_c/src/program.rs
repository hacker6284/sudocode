...
//! unit. Dependency modules' **value** symbols (functions, constants)
//! are qualified via `sudoc_ir::mangle::qualify_value`
//! (`sudo_M<len><module>_<len><name>`); qualified references
//! (`sorting.quicksort`) go through the same scheme. Record and enum
//! IR symbols are already unique; this pass does not rename types.
//! The entry module keeps bare names.
