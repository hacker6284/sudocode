...
//! boundary mapping), parallel assignments whose targets are not all plain
//! variables are lowered to temporaries (var-var stays a TupleAssign so a
//! local permutation can be a rebinding), and generics do not exist (the
//! checker monomorphizes generics away before IR — language §5.2).
