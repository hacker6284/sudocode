...
emitExpr ctx e = case eKind e of
  ...
  -- Rt.listOf WHNF-forces each element LTR before building the Seq (Stage A).
  EText cps ->
    "(Rt.listOf ([" ++ intercalate ", " (map show cps) ++ "] :: [Int64]))"
...
-- Multi-var payloads go through emitStrictTuple so each loop-carried variable
-- is WHNF-forced before the Cont/Brk tuple is built (avoids unbounded thunk
-- chains across iterations; Stage A).
varsTupleExpr :: [String] -> String
varsTupleExpr [] = "()"
varsTupleExpr [v] = mangleValue v
varsTupleExpr vs = emitStrictTuple (map mangleValue vs)
