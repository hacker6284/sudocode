...
# External backends driven by the protocol-4 emit path.
...
bench_hs() {
  ...
  # 2. protocol-4 emit request envelope (lockstep.bzl _external_codegen_impl)
  printf '{"protocol":4,"cmd":"emit","entry":"%s","with_tests":true,"modules":' "$program" >"${stage}/request.json"
